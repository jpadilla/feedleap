...
class Client:
    ...
    # Returns data on success, and false on failure.
    def getList(self, id):
        r = requests.get('https://kippt.com/api/lists/' + str(
    ...
    def getListCollab(self, id):
        r = requests.get('https://kippt.com/api/lists/' + str(
            id) + '/collaborators', headers=self.header)
        if r.status_code is 200:
            return r.json
        else:
            return False
    ...
    def getClips(self, listID=None, limit=0, offset=0):
        params = {"limit": str(limit),
                  "offset": str(offset)}
        url = 'https://kippt.com/api/clips?limit={limit}&offset={offset}'.format(
            **params)
    ...
    # Returns data on success, and false on failure.
    def getClip(self, id):
        r = requests.get('https://kippt.com/api/clips/' + str(
            id), headers=self.header)
        if r.status_code is 200:
            return r.json
        else:
    ...
    def search(self, query, limit=0, offset=0):
        ...
        r = requests.get(
            'https://kippt.com/api/search/clips/?q={query}&limit={limit}&offset={offset}'.format(
                **params),
            headers=self.header)
        if r.status_code is 200:
            return r.json['meta'], r.json['objects']
    ...
    def createList(self, name):
        clipdata = {'title': name}
        r = requests.post('https://kippt.com/api/lists/', data=json.dumps(
            clipdata), headers=self.header)
    ...
    def addClip(self, url, listID=0, title=None, starred=None, notes=None, ):
        ...
        r = requests.post('https://kippt.com/api/clips/', data=json.dumps(
            clipdata), headers=self.header)
        if r.status_code is 201:
            return r.json
        else:
    ...
    def deleteClip(self, id):
        r = requests.delete('https://kippt.com/api/clips/' + str(
            id), headers=self.header)
    ...
    def deleteList(self, id):
        r = requests.delete('https://kippt.com/api/lists/' + str(
            id), headers=self.header)
        if r.status_code is 204:
            return True
        else:
    ...
    def updateClip(self, id, title=None, notes=None, listID=None, starred=None):
        ...
        r = requests.put('https://kippt.com/api/clips/' + str(
            id), data=json.dumps(clipdata), headers=self.header)
        if r.status_code is 200:
            return True
        else:
            return False
    ...
    def updateList(self, id, title=None):
        ...
        if not title is None:
            clipdata['title'] = title
        r = requests.put('https://kippt.com/api/lists/' + str(
            id), data=json.dumps(clipdata), headers=self.header)
        if r.status_code is 200:
            return True
        else:
